...
epim <- readRDS("/Volumes/T7/longcovid/Gene_symbol_update/refup_duprm/epim2.rds")

epim=subset(epim, idents="Serous", invert=T)
levels(epim)

Idents(epim)="diagnosis"
Control=subset(epim, idents="Control")



data=GetAssayData(Control, assay = "RNA", slot = "data" )

Idents(Control)="celltype5"

labels <- Idents(Control)
table(Control$diagnosis)

metadata=Control@meta.data
...
pdf(file ="tgfbcho.pdf", width = 10, height =10)
...
ht1 + ht2
saveRDS(cellchat, file="Control_cellchat0.23.rds")

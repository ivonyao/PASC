...
top20 <- Macmarkers2 %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)

DoHeatmap(subset(Mac, downsample = 1000), features = top20$gene) +  scale_fill_gradientn(colors = c("lightblue", "white", "red"))


saveRDS(Macmarkers2, file="Macmarkers2.rds")

write.csv(Macmarkers2, "Macmarkers2.csv")
...
DimPlot(Mac, split.by="diagnosis")




Mac=RenameIdents(Mac, "MERTK+ Mac"="MERTK+ SPP1+ Mac", "SPP1+ Mac"="MERTK+ SPP1+ Mac")

Mac$celltype3=Idents(Mac)


levels(Mac)=c("FABP4+ Mac" ,   "FCN1+ CD16+ Mac", "FCN1+ CD14+ Mac", "Intermediate Mac", "MERTK+ SPP1+ Mac", "T monocyte complex" )

library(future)
options(future.globals.maxSize= 2000000000)
plan("multiprocess", workers = 4)

Macmarkers3=FindAllMarkers(Mac, only.pos = T)

top10 <- Macmarkers3 %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)

DoHeatmap(subset(Mac, downsample = 1000), features = top10$gene) +  scale_fill_gradientn(colors = c("lightblue", "white", "red"))
saveRDS(Macmarkers3, file="Macmarkers3.rds")
